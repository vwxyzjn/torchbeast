...
def act(
    ...
    policy_version_queue: mp.SimpleQueue,
    ...
    try:
        ...
        while True:
            ...
            # Do new rollout.
            policy_version = int(model.version)
            ...
            policy_version_queue.put(policy_version)
...
def get_batch(
    ...
    with lock:
        ...
        policy_versions = [policy_version_queue.get() for _ in range(flags.batch_size)]
    ...
    timings.time("device")
    return batch, initial_agent_state, policy_versions
...
def train(flags):  # pylint: disable=too-many-branches, too-many-statements
    ...
    policy_version_queue = ctx.Queue(maxsize=flags.batch_size * 2) # 2x batch size to test throughput
    ...
    step, stats = 0, {}
    policy_version2frequency = defaultdict(int)
    policy_update2policy_version = defaultdict(lambda: defaultdict(int))
    update = 1
    def batch_and_learn(i, lock=threading.Lock()):
        """Thread target for the learning process."""
        nonlocal step, stats, update
        timings = prof.Timings()
        while step < flags.total_steps:
            timings.reset()
            batch, agent_state, policy_versions = get_batch(
                flags,
                free_queue,
                full_queue,
                policy_version_queue,
            ...
            with lock:
                ...
                for policy_version in policy_versions:
                    ...
                    policy_version2frequency[policy_version] += 1
                    policy_update2policy_version[update][policy_version] += 1
